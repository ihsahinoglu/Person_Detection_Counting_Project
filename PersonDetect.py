    ...
    while True:
        ...
        if succes:  # eğer video başarılı bir şekilde okunabilirse
            ...
            for (objectId, bbox) in objects.items():
                ...
                tracking_line[objectId].append((centerX, centerY))
                ...
                if len(tracking_line[objectId]) > 1:
                    for j in range(len(tracking_line[objectId])-1):
                        cv2.line(frame, tracking_line[objectId][j], tracking_line[objectId][j+1], (255, 0, 0), 2)
                if objectId not in object_id_list:
                    object_id_list.append(objectId)
